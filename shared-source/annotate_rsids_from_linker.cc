...
void stream_update(const std::string &analysis_filename,
  ...
  fileinterface::fileinterface_reader *input_analysis = 0, *input_linker = 0;
  ...
  try {
    input_analysis = fileinterface::reconcile_reader(analysis_filename);
    input_linker = fileinterface::reconcile_reader(linker_filename);
