...
#ifndef __QSUB_JOB_MONITOR_UTILITIES_H__
#define __QSUB_JOB_MONITOR_UTILITIES_H__
...
namespace qsub_job_monitor {
  ...
  */
  ...
  /*!
    ...
    delimited by commas. In that case, each allele is split out and RCed separately,
    and the alleles are returned in the same order as input.
  */
  ...
  /*!
    ...
    @param total_bins total number of bins in which to approximately equipartition unit interval
    \return computed index on [0,total_bins)

    \warning behavior is to map 0 to the first bin, and then have every other bin be half-open lower
  */
  unsigned get_bin_index(const double &p,
			 unsigned total_bins);
