...
void fileinterface::fileinterface_writer_bzip2::open(const char *filename) {
  if (_raw_output)
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2: attempted to "
        "reopen "
        "in-use handle");
  _raw_output = fopen(filename, "w");
  if (!_raw_output)
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2: cannot open file "
        "\"" +
        std::string(filename) + "\"");
  int error = 0;
  ...
  if (error == BZ_CONFIG_ERROR) {
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2::open: bzip2 "
        "writing "
...
void fileinterface::fileinterface_writer_bzip2::close() {
  int error = 0;
  if (_bz_output) {
    BZ2_bzWriteClose(&error, _bz_output, 0, 0, 0);
    if (error == BZ_SEQUENCE_ERROR) {
      throw std::domain_error(
          "fileinterface::fileinterface_writer_bzip2::close: bzip "
          "reports "
...
void fileinterface::fileinterface_writer_bzip2::clear() {
  _good = true;
  _bad = _fail = false;
}

bool fileinterface::fileinterface_writer_bzip2::is_open() const {
  return (_raw_output && _bz_output);
}

void fileinterface::fileinterface_writer_bzip2::put(char c) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2::put: bzip reports "
        "write operation called on read handle");
  }
}

void fileinterface::fileinterface_writer_bzip2::writeline(
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2::writeline: bzip "
        "reports "
        "write operation called on read handle");
  }
}

void fileinterface::fileinterface_writer_bzip2::write(char *buf,
                                                      std::streamsize n) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "fileinterface::fileinterface_writer_bzip2::write: bzip "
        "reports "
        "write operation called on read handle");
  }
}
