...
void fileinterface::fileinterface_reader_flat::open(const char *filename) {
  _input.open(filename);
  if (!_input.is_open())
    throw std::domain_error(
        "fileinterface::fileinterface_reader_flat::open: cannot open "
        "file \"" +
        std::string(filename) + "\"");
}

bool fileinterface::fileinterface_reader_flat::getline(std::string *line) {
  if (!line) throw std::domain_error("flat::getline: called with null pointer");
  *line = "";
...
void fileinterface::fileinterface_reader_flat::read(char *buf,
                                                    std::streamsize n) {
  _input.read(buf, n);
}
