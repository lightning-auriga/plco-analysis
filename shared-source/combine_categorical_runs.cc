/*!
  \file combine_categorical_runs.cc
...
void process_data(
  ...
  std::vector<fileinterface::fileinterface_reader *> inputs;
  ...
  try {
    ...
    for (unsigned i = 0; i < input_filenames.size(); ++i) {
      inputs.at(i) = fileinterface::reconcile_reader(input_filenames.at(i));
    }
    ...
    for (std::vector<fileinterface::fileinterface_reader *>::iterator iter =
             inputs.begin();
...
void compute_combinatorial_uniques(
  ...
  for (unsigned i = 0; i < model_matrix_filenames.size(); ++i) {
    fileinterface::fileinterface_reader *input = 0;
    try {
      input = fileinterface::reconcile_reader(model_matrix_filenames.at(i));
      input->getline(&line);
